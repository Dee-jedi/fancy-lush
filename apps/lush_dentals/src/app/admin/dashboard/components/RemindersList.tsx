            ...
            >
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-emerald-900/5">
                <button
                  onClick={() => onSend(reminder)}
                  disabled={actionLoadingId === reminder.id}
                  className={`flex-1 max-w-[140px] py-2.5 rounded-xl text-[9px] tracking-widest font-black uppercase transition-all focus:outline-none cursor-pointer text-center ${
                    reminder.status === 'sent'
                      ? 'bg-emerald-950/5 text-[var(--primary)] hover:bg-emerald-950/10'
                      : 'bg-[var(--secondary)] text-[var(--primary)] hover:bg-[#b0913c] shadow-sm'
                  }`}
