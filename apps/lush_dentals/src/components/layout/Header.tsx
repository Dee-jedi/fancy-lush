  ...
  return (
    <>
      <motion.header 
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-2xl h-16 md:h-24 border-b border-emerald-900/5 shadow-sm flex items-center"
      >
        <div className="max-w-7xl mx-auto px-6 h-full w-full flex items-center justify-between">
          ...
          >
            <span className="text-2xl md:text-3xl font-serif font-bold tracking-widest leading-none text-(--primary)">
              LUSH <span className="text-(--secondary)">DENTALS</span>
            </span>
            <span className="text-[9px] tracking-[0.4em] uppercase font-medium mt-1.5 ml-0.5 text-(--foreground)/40">
              Transforming Smiles
            </span>
          </motion.a>
          
          {/* Desktop Nav */}
          <nav className="hidden lg:flex gap-12 text-[11px] font-bold uppercase tracking-[0.2em] text-(--foreground)/60">
            {activeMenuItems.map((item) => (
              ...
              >
                {item.name}
                <span className={`absolute bottom-0 left-0 h-px bg-(--secondary) transition-all ${
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="hidden lg:block z-80">
              {user ? (
                <div className="flex items-center gap-3">
                  ...
                  <button 
                    onClick={signOut} 
                    className="text-[10px] tracking-widest font-bold uppercase text-(--foreground)/40 hover:text-rose-500 transition-colors cursor-pointer focus:outline-none"
                  >
                    SIGN OUT
                  </button>
                </div>
            ...
            >
              ...
              <motion.span 
                ...
                className="w-8 h-[1.5px] bg-(--primary) rounded-full"
              />
              <motion.span 
                animate={isOpen ? { rotate: -45, y: -8, backgroundColor: "#16a07a" } : { rotate: 0, y: 0, backgroundColor: "#16a07a" }}
                className="w-8 h-[1.5px] rounded-full"
        ...
        {isOpen && (
          <>
            <motion.div 
              ...
              className="fixed top-16 md:top-24 bottom-0 left-0 right-0 bg-black/10 backdrop-blur-sm z-60"
            />
            <motion.div 
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              ...
              className="fixed right-0 top-16 md:top-24 bottom-0 w-[75%] sm:w-[400px] bg-white/80 backdrop-blur-2xl z-70 shadow-[-20px_0_60px_rgba(0,0,0,0.03)] p-8 md:p-12 flex flex-col justify-between border-t border-gray-50"
            >
                    ...
                    return (
                      <div key={item.name} className="flex flex-col items-start gap-1">
                        <motion.a
                          ...
                          onClick={() => setIsOpen(false)}
                          className={`text-2xl md:text-3xl font-serif font-bold uppercase tracking-wider ${isActive ? 'text-(--primary)' : 'text-(--foreground)/40'} hover:text-(--primary) transition-all duration-300`}
                        >
                          {item.name}
                        </motion.a>
                        {isActive && (
                          <motion.div 
                            ...
                            className="w-8 h-px bg-(--secondary) rounded-full mt-1"
                    ...
                    {user ? (
                      <>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] tracking-widest font-bold text-emerald-950/40 uppercase">
